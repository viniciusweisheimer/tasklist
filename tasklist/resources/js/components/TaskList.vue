<template>
    <div class='row'>
        ...
        <h4>All Tasks</h4>
        <ul class="list-group">
            <li v-if='list.length === 0'>No tasks available</li>        
...
<script>
    export default {
        ...
        methods: {
            fetchTaskList() {
                axios.get('api/tasks').then((res) => {this.list = res.data;});
            },
            ...
            deleteTask(id) {
                axios.delete('api/task/' + id).then((res) => {this.fetchTaskList()}).catch((err) => console.error(err));
            },
